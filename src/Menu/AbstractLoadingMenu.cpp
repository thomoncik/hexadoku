...
void AbstractLoadingMenu::OnEntry(Game &game) {
...
void AbstractLoadingMenu::Update(Game &game) {

}

void AbstractLoadingMenu::HandleInput(Game &game, char input) {
    if (input == 'j') {
        option++;
        ...
        auto board = std::make_shared<Board>(Board::STANDARD_SIZE);
        board->LoadFromFile(option->first);

        game.SetState(std::make_shared<MovePlayState>(Board::STANDARD_SIZE, board));
    }
}

void AbstractLoadingMenu::Draw(Game &game) {
    gfx::out << gfx::clear;
...
void AbstractLoadingMenu::OnExit(Game &game) {
